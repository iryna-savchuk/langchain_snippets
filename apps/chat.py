...
active_loop_data_set_path = f"hub://{my_activeloop_org_id}/{my_activeloop_dataset_name}"
...
# Record audio using audio_recorder and transcribe using 
def record_and_transcribe_audio():
    ...
    if audio_bytes:
        ...
        if st.button("Transcribe"):
            transcription = transcribe_audio(TEMP_AUDIO_PATH, os.environ["OPENAI_API_KEY"])
...
def display_transcription(transcription):
    if transcription:
        ...
        with open("../output/audio_transcription.txt", "w+") as f:
...
def display_conversation(history):
    for i in range(len(history["generated"])):
        ...
        audio = generate(text=text, voice=voice,api_key=os.environ["ELEVEN_API_KEY"])
